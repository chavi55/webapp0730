...
# 2. 데이터 불러오기 (캐싱을 통해 속도 최적화)
@st.cache_data
def load_population_data():
    """인구 CSV 데이터를 불러오고 최신 연도 데이터를 정제하는 함수"""
    url = "https://raw.githubusercontent.com/greatsong/modudata/main/data/population_yearly.csv.gz"
    
    # '코드' 열은 반드시 문자열(string) 형태(10자리)로 읽어옵니다.
    df = pd.read_csv(url, dtype={'코드': str})
    
    # 코드가 10자리가 안 될 경우 앞에 0을 채워줍니다 (예: '01234' -> '0123456789')
...
fig = px.choropleth_mapbox(
    ...
    center={"lat": 35.9, "lon": 127.7},  # 대한민국 중심 좌표
    zoom=6,
    opacity=0.85
)
...
# 툴팁(마우스 오버) 라벨 한글화
fig.update_traces(
    hovertemplate="<b>%{hovertext}</b><br>시군구코드: %{location}<br>중학생 비율: %{customdata[1]:.2f}%<br>중학생 수: %{customdata[2]:,}명<br>전체 인구: %{customdata[3]:,}명<br><extra></extra>"
)
...
# 애니메이션(월별 데이터)이 있을 경우 가장 최근 월 데이터로 순위 추출
